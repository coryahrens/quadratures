       subroutine eqquadpw(nrhs,nnmax,ngen,gen,wei,wvert,wface,wside, 
...
c  
         do i=1,neq
           ...
           fval(i) = vval(i)*exp(wvert)
	   
         enddo 
c
c--------Jacobian (derivative wrt "wvert" in the last column)
c	 
         call toarray(neq,nrhs,jac,1,nrhs,cvert*exp(wvert))
c	 
         do i=2,neq
	 
           call toarray(neq,nrhs,jac,i,nrhs,vval(i)*exp(wvert))
...
c
           do i=1,neq
             ...
             fval(i) = fval(i) + cval(i) * exp(wface)
	     
           enddo
c
c--------Jacobian (derivative wrt "wface" in the second to last column)
c
           call toarray(neq,nrhs,jac,1,nrhs-1,cface*exp(wface))
c	 
           do i=2,neq
	   
             call toarray(neq,nrhs,jac,i,nrhs-1,cval(i)*exp(wface))
...
c
           do i=1,neq
             fval(i) = fval(i) + sval(i) * exp(wside)
           enddo
c
c----------Jacobian (derivative wrt "wside" in the second to last column)
c
           call toarray(neq,nrhs,jac,1,nrhs-1,cside*exp(wside))
c	 
           do i=2,neq
             call toarray(neq,nrhs,jac,i,nrhs-1,sval(i)*exp(wside))
           enddo
...
c
           call toarray(neq,nrhs,jac,1,nrhs-2,cface*exp(wface))
c	 
           do i=2,neq
             call toarray(neq,nrhs,jac,i,nrhs-2,cval(i)*exp(wface))
           enddo	   
...
c----------Jacobian (derivative wrt "wside" in the second to last column)
c
           call toarray(neq,nrhs,jac,1,nrhs-1,2.0d0*cside*exp(wside))
c	 
           do i=2,neq
             call toarray(neq,nrhs,jac,i,nrhs-1,sval(i)*exp(wside))
           enddo
...
c----------If so, use the other 60 points generated by reflctions
c     
	   if(ifg.eq.1) then
...
c
           do i=1,neq
             ...
             fval(i) = fval(i) + qval(i)*exp(wei(j))
	     
           enddo   
c
c----------derivative wrt "theta" in 1-st position, wrt "phi" in 2-nd position, wrt "wei" in 3-rd position 
c----------partial wrt weights of first equation (0,0)th 
c
CCCCCCC
CCCCCCC--STARTED CHANGING VARIABLE ORDERING 2/3/2010
           if(ifg.eq.0) then
	   
c             call toarray(neq,nrhs,jac,1,3*j,cgene*exp(wei(j)))
	     call toarray(neq,nrhs,jac,1,2*ngen + j,cgene*exp(wei(j)))
	     
	   else
	   
c	     call toarray(neq,nrhs,jac,1,3*j,cgenefg*exp(wei(j)))
	     call toarray(neq,nrhs,jac,1,2*ngen + j,cgenefg*exp(wei(j)))
...
c	     call toarray(neq,nrhs,jac,i,3*j,qval(i)*exp(wei(j)))	     
             call toarray(neq,nrhs,jac,i,2*ngen + j,qval(i)*exp(wei(j)))
...
c             call toarray(neq,nrhs,jac,i,3*j-1,pdph(i)*exp(wei(j)))
c             call toarray(neq,nrhs,jac,i,3*j-2,pdth(i)*exp(wei(j)))

             call toarray(neq,nrhs,jac,i,ngen + j,pdph(i)*exp(wei(j)))
             call toarray(neq,nrhs,jac,i,       j,pdth(i)*exp(wei(j)))
