       subroutine eqquadw(nrhs,nnmax,ngen,gen,wei,wvert,wface,wside, 
     1               fval,jac,neq,ipn,ipm,itype,ifg,iter)
c=======================================================================c 
...
c
                 qval(i) = qval(i) + p(ipm(i),ipn(i))*phire(ipm(i))
c                    
                 pdth(i) = pdth(i) +
     1           dp(ipm(i),ipn(i))*phire(ipm(i))*ftheta1(k) -
     2           ipm(i)*p(ipm(i),ipn(i))*phiim(ipm(i))*ftheta2(k)               
c                        
                 pdph(i) = pdph(i) +
     1           dp(ipm(i),ipn(i))*phire(ipm(i))*fphi1(k) -
...
c     
	   if(ifg.eq.1) then
c
c------------Use quaternion multiplication to generate the 60 images of rgen(j)
c	 	       
             do k=1,60
               call quatmult(rgen(1,j),rgc(1,k),qtemp(1,k))
               call quatmult(rg(1,k),qtemp(1,k),rqres(1,k))
             enddo
c
c------------sum over the 60 images from the reflections
c
             do k=1,60
               ct    = rqres(4,k)
	       theta = acos(ct)
c           
	       if((rqres(2,k)*rqres(2,k) + rqres(3,k)*rqres(3,k)) .le. 
     1	           1.0d-12) then
	         si = 0.0d0
	         co = 1.0d0
	       else
                 sr = sqrt(rqres(2,k)*rqres(2,k) + 
     1		           rqres(3,k)*rqres(3,k))
                 si = qres(3,k)/sr
                 co = qres(2,k)/sr	     
	       endif
c  
...
c
           if(ifg.eq.zero) then
             ...
             call toarray(neq,nrhs,jac,1,3*j,cgene*exp(wei(j)))
	     
	   else
	   
	     call toarray(neq,nrhs,jac,1,3*j,cgenefg)
...
c
	     ...
	     call toarray(neq,nrhs,jac,i,3*j,qval(i)*exp(wei(j)))	     
...
c
           do i=1,neq
             ...
             call toarray(neq,nrhs,jac,i,3*j-1,pdph(i)*exp(wei(j)))
             call toarray(neq,nrhs,jac,i,3*j-2,pdth(i)*exp(wei(j)))
