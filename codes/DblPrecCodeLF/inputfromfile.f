       subroutine inputfromfile(ngenmax,nmax,ngen,theta0,phi0,wei0,
	 ...
	 real*8 gen(4,ngenmax), sr, cphi, lambda, thres
	 
	 integer itype, ifg, ngen, nmax, nvar, iweights
...
c	 	 
	 if(io.gt.0) then
	   ...
	   call prina('Quadrature weight file name: *',wfname,l)

           call makefilename(nmax,ngen,itype,nodes,gfname,l,1)	   
	   call prina('Quadrature gen file name: *',gfname,l)
...
CCC	 call prinf('Generator file name:*',0,0)
CCC	 read(7,*) gdata

	 call makefilename(nmax,ngen,itype,nodes,gdata,l,3)

	 call openfile(gdata,8,'old')
...
c
	   do j=1,ngen
	     gen(1,j) = zero
	     read(8,*)  gen(2,j), gen(3,j), gen(4,j) 
	   enddo
	   close(8)	 
...
c
	       do j=1,ngen
	         gen(1,j) = zero
	         read(8,*)  gen(2,j), gen(3,j), gen(4,j), wei0(j)  
	       enddo     	     
